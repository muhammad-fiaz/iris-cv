...
In Observers, images are represented by the `Image<B>` struct, where `B` is a Burn `Backend` type. 
...
Observers uses the `image` crate internally to handle file decoders.
